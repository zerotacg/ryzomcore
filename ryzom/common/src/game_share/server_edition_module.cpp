...
//		typedef CSessionId TSessionId; //		typedef uint32 TSessionId;
	...
	public:
		...
		CEditionSession(RSMGR::TSessionType type = RSMGR::TSessionType::invalid_val, TSessionId sessionId = TSessionId(0), TCharId ownerCharId = 0, TAiInstanceId aiInstanceId = 0, CScenario* scenario = nullptr);
...
void CServerEditionModule::init(NLNET::IModuleSocket* gateway, CDynamicMapService* server)
{
	...
	resetKeyPolicies(nullptr);
	setDefaultKey(nullptr, "DEV");
	addKeyPolicy(nullptr, "DEFAULT", "EMPTY", "ACCEPT");  //Accept could come from live
	addKeyPolicy(nullptr, "EMPTY", "EMPTY", AcceptScenarioWithoutHeader.get()?"ACCEPT":"REFUSE"); // Deprecated (scenario older than 22 juin 2006)
	addKeyPolicy(nullptr, "DEV", "DEV:26289fe9502f4db1fa3229e4fb2d8803", "TEST");
...
void CServerEditionModule::onModuleUp(NLNET::TModuleProxyPtr senderModuleProxy)
{
	...
	else if ( moduleName == "CharacterControl")
	{
		...
		auto first(_Sessions.begin()), last(_Sessions.end());
		for (; first != last; ++first)
		{
			CEditionSession *session = first->second;

			auto f2(session->getCurrentChars().begin()), e2(session->getCurrentChars().end());
			for (; f2 != e2; ++f2)
			{
	...
	else if (moduleName == "ClientEditionModule")
	{
		...
		bool ok = getCharInfo(senderModuleProxy.get(), charId, clientEid, userPriv, extendedPriv);
		...
		if (_ClientsEditionModule.getA(senderModuleProxy) != nullptr)
		{
		...
		if (_ClientsEditionModule.getB(charId) != nullptr)
		{
...
void CServerEditionModule::connectChar(TSessionId sessionId, TCharId charId, TUserRole userRole, const std::string& ringAccess, bool newcomer)
{
	...
	*/
	...
	// If its a dev he has specials ring Acess
	{
		...
		bool ok = getCharInfo(clientEditionProxy.get(), charId, clientEid, userPriv, extendedPriv);
		if (ok)
		{
...
void CServerEditionModule::disconnectChar(TCharId charId)
{
	...
	DROP_IF(_CharacterControlProxy == nullptr, "Try to disconnect a player but no EGS?", return);
	CCharacterControlItfProxy proxy(_CharacterControlProxy);
...
void CServerEditionModule::onModuleDown(NLNET::TModuleProxyPtr moduleProxy)
{
	...
	{
		_R2SessionBackupModule = nullptr;
	}
	...
	else if (moduleName == "ClientEditionModule")
	{
		...
		if (_ClientsEditionModule.getA(moduleProxy) != nullptr)
		{
			...
			// Do not useeckSecurityInfo because we put the client in authorized list here.
			bool ok = getCharInfo(moduleProxy.get(), charId, clientEid, userPriv, extendedPriv);
...
void CServerEditionModule::onMapConnectionAsked( NLNET::TModuleProxyPtr clientEditionProxy,
	TSessionId sessionId, bool updateHighLevel, bool mustTp, TUserRole role)
{
	...
	bool ok = checkSecurityInfo(clientEditionProxy.get(), charId, clientEid, userPriv, extendedPriv);
	if (!ok) { return; }


	CEditionSession* session = getSession(sessionId);
	CScenario* scenario = session ? session->getScenario() : nullptr;

	auto previousSessionId = TSessionId(0);
	if (!session ||!scenario )
	{
		return;
	}

	auto previousSessionIdIt = _PioneersSessions.find(charId);
...
void CServerEditionModule::broadcastToCharConnected(const NLNET::CMessage & msg, CEditionSession* session)
{
	...
	auto first(broadcastList.begin()), last(broadcastList.end());
	for (;first != last ; ++first)
	{
...
void CServerEditionModule::fillBroadcastVector(CEditionSession * session, std::vector<NLNET::TModuleProxyPtr> &broadcastList)
{
	...
	auto first(session->getCurrentChars().begin()), last(session->getCurrentChars().end());
	for (;first != last ; ++first)
	{
...
void CServerEditionModule::onScenarioUploadAsked(NLNET::TModuleProxyPtr senderModuleProxy, uint32 msgId,
	const CObjectSerializerServer& hlScenario, bool mustBroadCast)
{
	...
	if (mustBroadCast)
	{
		...
		vector<TModuleProxyPtr> otherThantSender = removeSender(broadcastList, senderModuleProxy.get());
...
void CServerEditionModule::onNodeInsertAsked(NLNET::TModuleProxyPtr senderModuleProxy, uint32 messageId,
	...
	const std::string& key, const CObjectSerializerServer& value2)
{
	...
	vector<TModuleProxyPtr> broadcastList;
	fillBroadcastVectorNoSender(session, broadcastList, senderModuleProxy.get());
...
void CServerEditionModule::onNodeSetAsked(NLNET::TModuleProxyPtr senderModuleProxy, uint32 messageId,
...
void CServerEditionModule::onNodeEraseAsked(NLNET::TModuleProxyPtr senderModuleProxy, uint32 messageId,
	const std::string& instanceId, const std::string& attrName, sint32 position)
{
...
void CServerEditionModule::onNodeMoveAsked(NLNET::TModuleProxyPtr senderModuleProxy, uint32 messageId,
...
void CServerEditionModule::onUserComponentUploaded(NLNET::IModuleProxy *senderModuleProxy, CUserComponent* component)
{
	...
	CUserComponentRefCounter* refCounter = nullptr;
	auto found = _UserComponentRefCounter.find(component->Md5);
...
CUserComponent* CServerEditionModule::getUserComponent( const NLMISC::CHashKeyMD5& md5) const
{
	...
	auto found = _UserComponentRefCounter.find(md5);
	if (found != _UserComponentRefCounter.end())
	{
...
void CServerEditionModule::onUserComponentRegistered(NLNET::TModuleProxyPtr senderModuleProxy, const NLMISC::CHashKeyMD5& md5 )
{
	TCharId charId;
	CEntityId clientEid;
	std::string userPriv;
...
bool CServerEditionModule::onProcessModuleMessage(NLNET::TModuleProxyPtr senderModuleProxy, const NLNET::CMessage &message)
{
	...
	if (senderModuleProxy->getModuleClassName() == "ClientEditionModule")
	{
		...
		if (operationName == "SUCU")
		{
			...
			onUserComponentUploaded(senderModuleProxy.get(), component);
			return true;
		}

		if (operationName == "DBG_CREATE_PRIMITIVES")
		{
			onMessageReceivedCreatePrimitives(senderModuleProxy.get(), charId );
			return true;
		}

		if (operationName == "STOP_TEST")
		{
			stopTestRequested(senderModuleProxy.get(), charId);
			return true;
		}
		...
		if (operationName == "requestUpdateRtScenario")
		{
			...
			rtScenarioUpdateRequested(senderModuleProxy.get(), charId, obj.getData());
...
CEditionSession* CServerEditionModule::getSession(TSessionId sessionId) const
{
	...
	if (found == _Sessions.end()) return nullptr;
	return found->second;
}
...
{
	TPioneersSessionsAlloweds::const_iterator found = _PioneersSessionsAllowed.find(charId);
	if (found == _PioneersSessionsAllowed.end()) return nullptr;
...
void CServerEditionModule::createSession(NLNET::IModuleProxy *sender, TCharId ownerCharId, TSessionId sessionId, const RSMGR::TSessionType &type)
{
	...
	if (type.getValue() == RSMGR::TSessionType::st_edit)
	{
		...
		CScenario* scenario = new CScenario(nullptr, st_edit);
	...
	else if (type.getValue() == RSMGR::TSessionType::st_anim)
	{
		...
		if (!editSession)
		{
			...
			CScenario* scenario = new CScenario(nullptr, st_anim);
			scenario->setMode(0);
...
//			info.setMissionTag(missionTag);
			...
			if (ownerModule)
			{
				...
				std::string userPriv;
				getCharInfo(ownerModule->get(), charId, clientEid, userPriv, extendedPriv);
			}
...
void CServerEditionModule::verifyRingAccess(TSessionId sessionId, TCharId charId)
{
	...
	if (found != _PioneersInfo.end())
	{
		...
		{
			 resetSession(nullptr, sessionId, false); //TODO kick if invited, reset if owner
		}
	}
}
...
bool CServerEditionModule::teleportOneCharacterToAnotherImpl(TSessionId sessionId, TCharId source, TCharId dest, std::string&msg)
{
	...
	if (_CharacterControlProxy == nullptr)
	{
...
void CServerEditionModule::startScenario(NLNET::IModuleProxy *senderModuleProxy, bool ok, const TScenarioHeaderSerializer& header, const CObjectSerializerServer &data, uint32 startingAct)
{
	...
	{
		CVerfiyRightRtScenarioError *err = nullptr;
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, displayPioneer)
{
	...
	log.displayNL("'%s' '%s'",
		NLMISC::toString("Chars: %u, IsConnected %u", found->first, (getClientProxyPtr(found->first) != nullptr)).c_str(),
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, resetSession)
{
	...
	uint32 sessionId;
	fromString(args[0], sessionId);
	this->resetSession(nullptr, (TSessionId)sessionId, false);
	return true;
}
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, reconnectSession)
{
	...
	uint32 sessionId;
	fromString(args[0], sessionId);
	this->resetSession(nullptr, (TSessionId)sessionId, true);
	return true;
}
...
void CServerEditionModule::onCharModeUpdateAsked(NLNET::TModuleProxyPtr senderModuleProxy, TCharMode mode)
{
	TCharId charId;
	CEntityId clientEid;
	std::string userPriv;
...
void CServerEditionModule::characterReady(TCharId charId)
{
	...
	if (session->getOwnerCharId() == charId && _IOSRingProxy != nullptr)
	{
...
bool CServerEditionModule::isInitialized() const
{
	...
	if (DssUseBs) { return _CharacterControlProxy != nullptr && !_WaitingForBS;}


	return _CharacterControlProxy != nullptr;
}
			...
			for ( ;first !=  last; ++first )
			{
				...
				if (session)
				{
					...
					else if (session->mustClose(nowSeconds) )
					{
						...
						closeSession(nullptr, sessionId);
						// session is remove from _Sessions here
					}
...
{
	return getClientProxyPtr(charId) != nullptr;
}
...
void CServerEditionModule::onScenarioRingAccessUpdated(NLNET::TModuleProxyPtr senderModuleProxy, bool ok,const std::string & ringAccess, const std::string& errMsg)
{
	TCharId charId;
	NLMISC::CEntityId clientEid;
	std::string userPriv;
...
void CServerEditionModule::saveScenarioFile(NLNET::TModuleProxyPtr sender, const std::string &md5, const TScenarioHeaderSerializer &header)
{
	TCharId charId;
	NLMISC::CEntityId clientEid;
	std::string userPriv;
	...
	if (! checkSecurityInfo(sender.get(), charId, clientEid, userPriv, extendedPriv) ) { return; }
...
void CServerEditionModule::loadScenarioFile(NLNET::TModuleProxyPtr senderModuleProxy, const std::string &md5, const std::string &signature)
{
	TCharId charId;
	NLMISC::CEntityId clientEid;
	std::string userPriv;
...
void CServerEditionModule::saveUserComponentFile(TModuleProxyPtr sender, const std::string &md5, const TScenarioHeaderSerializer &header)
{
	TCharId charId;
	NLMISC::CEntityId clientEid;
	std::string userPriv;
...
// a message to validate a file waiting to be loaded
void CServerEditionModule::loadUserComponentFile(TModuleProxyPtr senderModuleProxy, const std::string &md5, const std::string &signature)
{
	TCharId charId;
	NLMISC::CEntityId clientEid;
	std::string userPriv;
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, addKeyPolicy)
{
	...
	addKeyPolicy(nullptr, args[0], args[1], args[2]);
	return true;
}
...
NLMISC_CLASS_COMMAND_IMPL(CServerEditionModule, setSessionStartParams)
{
	...
	setSessionStartParams(nullptr, charId, (TSessionId)sessionId, initialIslandLocation, initialEntryPoint, initialSeason);
	return true;
}
...
void CServerEditionModule::teleportWhileUploadingScenario(NLNET::IModuleProxy *sender, const std::string& initialIslandLocation, const std::string& initialEntryPoint, const std::string& initialSeason)
{
	...
	for (;first != last ; ++first)
	{
		...
		if (foundPioneersEId != _PioneersInfo.end())
		{
			...
			getTpContext(charId, tpInfos.TpCancelTextId, tpInfos.TpContext );
			tpPosition(nullptr, foundPioneersEId->second.EntityId, float(x), float(y), 0, season, tpInfos);
		}
	}
}
...
void CServerEditionModule::getStartParamsImpl(uint32 charId, TSessionId lastStoredSessionId)
{
	...
	CScenario * scenario = getScenarioById(sessionId);
	if (!scenario || scenario->getHighLevel() == nullptr) { reloadPos = false; }
...
// Simulate the dss
void CServerEditionModule::forwardToDss(TModuleProxyPtr senderModuleProxy, uint32 charId, const NLNET::CMessage &msg)
{
