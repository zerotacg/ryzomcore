...
PFNGLGETACTIVEUNIFORMPROC						nglGetActiveUniform;
PFNGLGETACTIVEUNIFORMSIVPROC					nglGetActiveUniformsiv;
...
PFNGLGENVERTEXARRAYSPROC						nglGenVertexArrays;
PFNGLDELETEVERTEXARRAYSPROC						nglDeleteVertexArrays;
PFNGLBINDVERTEXARRAYPROC						nglBindVertexArray;

PFNGLBINDBUFFERPROC								nglBindBuffer;
PFNGLBINDBUFFERBASEPROC							nglBindBufferBase;
PFNGLGETUNIFORMBLOCKINDEXPROC					nglGetUniformBlockIndex;
PFNGLUNIFORMBLOCKBINDINGPROC					nglUniformBlockBinding;
...
PFNGLCOPYBUFFERSUBDATAPROC						nglCopyBufferSubData;
...
PFNGLPROGRAMPARAMETERIPROC					nglProgramParameteri;
...
static bool setupGLCore(std::vector<const char *> &glext)
{
	...
	CHECK_ADDRESS(PFNGLGETACTIVEUNIFORMPROC, glGetActiveUniform);
	CHECK_ADDRESS(PFNGLGETACTIVEUNIFORMSIVPROC, glGetActiveUniformsiv);
	...
	CHECK_ADDRESS(PFNGLGENVERTEXARRAYSPROC, glGenVertexArrays);
	CHECK_ADDRESS(PFNGLDELETEVERTEXARRAYSPROC, glDeleteVertexArrays);
	CHECK_ADDRESS(PFNGLBINDVERTEXARRAYPROC, glBindVertexArray);

	CHECK_ADDRESS(PFNGLBINDBUFFERPROC, glBindBuffer);
	CHECK_ADDRESS(PFNGLBINDBUFFERBASEPROC, glBindBufferBase);
	CHECK_ADDRESS(PFNGLGETUNIFORMBLOCKINDEXPROC, glGetUniformBlockIndex);
	CHECK_ADDRESS(PFNGLUNIFORMBLOCKBINDINGPROC, glUniformBlockBinding);
	...
	CHECK_ADDRESS(PFNGLCOPYBUFFERSUBDATAPROC, glCopyBufferSubData);
...
static bool setupARBSeparateShaderObjects(std::vector<const char *> &glext)
{
	...
	CHECK_ADDRESS(PFNGLPROGRAMPARAMETERIPROC, glProgramParameteri);
