...
public:
	...
	void onModuleUp(NLNET::TModuleProxyPtr moduleProxy) NL_OVERRIDE
	{
		ModuleUpCalled++;

		if (moduleProxy->getModuleClassName() == getModuleClassName())
			ModuleType0.insert(moduleProxy.get());
	}
	 ...
	 */
	void onModuleDown(NLNET::TModuleProxyPtr moduleProxy) NL_OVERRIDE
	{
		ModuleDownCalled++;

		if (moduleProxy->getModuleClassName() == getModuleClassName())
			ModuleType0.erase(moduleProxy.get());
	}

	bool onProcessModuleMessage(NLNET::TModuleProxyPtr senderModuleProxy, const NLNET::CMessage &message) NL_OVERRIDE
	{
